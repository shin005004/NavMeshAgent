...
namespace RTS_Cam
{
    ...
    public class RTS_Camera : MonoBehaviour
    {
        ...
        public float maxZoom = 5f; //maximal height
        public float minZoom = 15f; //minimnal height
        public float zoomDamping = 1.0f;
        public float keyboardZoomingSensitivity = 2f;
        public float scrollWheelZoomingSensitivity = 25f;

        private float zoomSize = 5.0f; //value in range (0, 1) used as t in Matf.Lerp
        private float zoomSizeTarget = 0; // for smooth animation
        ...
        private void ZoomCalculation()
        {
            ...
            if (useScrollwheelZooming)
                zoomSizeTarget += ScrollWheel * Time.deltaTime * scrollWheelZoomingSensitivity;
            if (useKeyboardZooming)
                zoomSizeTarget += ZoomDirection * Time.deltaTime * keyboardZoomingSensitivity;

            zoomSizeTarget = Mathf.Clamp01(zoomSizeTarget);
            float zoomSizeTo = Mathf.Lerp(maxZoom, minZoom, zoomSizeTarget);

            zoomSize = zoomSizeTo * (1 - zoomDamping) + zoomSize * zoomDamping;

            m_Camera.orthographicSize = zoomSize;
        }
