...
{
    [CustomEditor(typeof(RTS_Camera_Pers))]
    public class RTS_CameraEditor : Editor
    {
        private RTS_Camera_Pers camera { get { return target as RTS_Camera_Pers; } }
        ...
        private void RotationTab()
        {
            using (new HorizontalBlock())
            {
                GUILayout.Label("Perspective on Floor: ", EditorStyles.boldLabel, GUILayout.Width(170f));
                camera.useFloorPerspective = EditorGUILayout.Toggle(camera.useFloorPerspective);
            }
            if(camera.useFloorPerspective)
            {
                GUILayout.Label("Toggle Button: ", EditorStyles.boldLabel, GUILayout.Width(170f));
                camera.toggleRotationKey = (KeyCode)EditorGUILayout.EnumPopup(camera.toggleRotationKey);
                camera.m_CameraRig = EditorGUILayout.ObjectField("Pivot: ", camera.m_CameraRig, typeof(Transform), true) as Transform;
                camera.movementTime = EditorGUILayout.FloatField("MovementTime: ", camera.movementTime);
            }
        ...
        private void ZoomTab()
        {
            ...
            camera.zoomTime = EditorGUILayout.FloatField("Zoom sensitivity: ", camera.zoomTime);
        }
    }
}